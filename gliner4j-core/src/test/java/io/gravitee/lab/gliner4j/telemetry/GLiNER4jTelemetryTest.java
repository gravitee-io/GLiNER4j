...
class GLiNER4jTelemetryTest {
  ...
  void recordsCorrectDuration() {
    var telemetry = new GLiNER4jTelemetry();
    telemetry.recordExtract(42.5, 1, 0);
  ...
  void recordsTextCount() {
    var telemetry = new GLiNER4jTelemetry();
    telemetry.recordExtract(10.0, 5, 0);
  ...
  void recordsEntityCount() {
    var telemetry = new GLiNER4jTelemetry();
    telemetry.recordExtract(10.0, 1, 7);

    var entityCount = findMetric("gliner4j.extract.entity.count");
    assertThat(entityCount).isNotNull();
    var sumData = entityCount.getLongSumData();
    assertThat(sumData.getPoints()).hasSize(1);
    var point = sumData.getPoints().iterator().next();
  ...
  void accumulatesAcrossMultipleCalls() {
    var telemetry = new GLiNER4jTelemetry();
    telemetry.recordExtract(10.0, 1, 3);
    telemetry.recordExtract(20.0, 2, 5);
    ...
    var entityCount = findMetric("gliner4j.extract.entity.count");
    var ePoint = entityCount.getLongSumData().getPoints().iterator().next();
